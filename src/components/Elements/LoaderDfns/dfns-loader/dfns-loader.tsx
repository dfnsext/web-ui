...
export class DfnsLoader {
	...
	render() {
		const primaryColor = dfnsStore.state.colors ? dfnsStore.state.colors.primary_500 : "#FFFFFF";
		const LoaderIconSrc: JSX.Element = (
			<svg
				...
				<path
					...
					fill={this.size === "small" && dfnsStore.state.theme.includes("dark") ? "#FFFFFF" : primaryColor}
					fill-opacity="0.1"
				/>
				<path
